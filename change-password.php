...
if(isset($_POST['submit']))
{
	...
	if($_POST['npass'] != $_POST['cfpass']) {
		...
		$row = hms_fetch_array(hms_query($con, "SELECT password FROM users WHERE id='".$userId."' LIMIT 1"));
		$stored = $row['password'] ?? '';
		...
		if ($isCurrentValid) {
			...
			$updateResult = hms_query($con,"update users set `password`='".hms_escape($con, $newHash)."', `updationDate`='$currentTime' where id='".$userId."'");
